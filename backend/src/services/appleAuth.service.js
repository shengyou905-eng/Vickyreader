...
function ensureAppleConfigured() {
  ...
  if (missing.length > 0) {
    throw new Error(`Apple authentication is not configured: ${missing.join(', ')}`);
  }
}
...
async function postAppleForm(url, form) {
  ...
  if (!response.ok || data.error) {
    throw new Error(`Apple token endpoint rejected the request: ${data.error || response.status}`);
  }
  return data;
}
