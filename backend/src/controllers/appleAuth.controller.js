...
const { hashRateLimitKey } = require('../services/passwordReset.service');
...
function createAppleAuthController(dependencies = {}) {
  const repository = dependencies.repository || authSecurityRepository;
  ...
  async function prepare(req, res, next) {
    try {
      ...
      const allowed = await repository.consumeRateLimit({
        ...
        keyHash: hashRateLimitKey(`ip:${ip}`, passwordResetRateLimitSecret),
        windowSeconds: 3600,
        limit: 30,
      });
