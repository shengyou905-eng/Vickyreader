...
1. 先读 `docs/zhidu_product_direction.md`。知读是围绕阅读痕迹形成的安静精神空间，不是 AI 效率工具。
2. 当前优先级固定为：阅读器稳定性 → 明台公共书库 → 小U人格化回顾 → 主题关联图谱 → 长期记忆结构。
3. 阅读器优先级最高。任何影响打开速度、翻页、章节切换、选区、进度恢复的改动，都要优先验证。
4. 明台是公共可阅读书库，不是社交 feed。不要新增热榜、点赞、关注、粉丝、无限流或推荐算法。
5. 小U是阅读意识层，不要做通用聊天机器人或数据报表。不要每次全量记录喂给大模型。
6. 随心记是绝对私密空间，永远不进明台，默认不被小U读取，不接 AI 分析和自动标签。

---

## 四、修改代码前

1. 读 `docs/zhidu_product_direction.md` 和 `ai_reading_app_product_structure_cn.md` 了解产品定位
...
| 明台新增 feed / 热榜 / 点赞 / 关注 | 明台是公共书库，不是社交平台 |
| 私人小U内容默认公开 | 必须由用户显式选择公开 |
| 小U实时全量喂给大模型 | 必须走结构化缓存和分层记忆 |
